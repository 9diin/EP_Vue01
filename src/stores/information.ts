...
// Pinia는 Vue.js의 상태 관리 라이브러리로, 애플리케이션의 상태를 중앙에서 관리하고 컴포넌트 간에 데이터를 공유할 수 있습니다.
import { defineStore } from "pinia";

// HTTP 요청을 위해 axios 라이브러리로 가져옵니다.
// axios는 브라우저와 Node.js 환경 모두 사용 가능한 Promise 기반의 HTTP 클라이언트 입니다.
import axios from "axios";

// 'main'이라는 이름의 스토어를 정의합니다.
// 스토어는 애플리케이션의 상태를 중앙에서 관리하며, 상태 변화에 대한 로직을 캡슐화 합니다.
export const useStore = defineStore("main", {
    // 스토어의 상태(state)를 정의합니다.
    state: () => ({
        // 기본 검색어로 "korea"를 설정합니다.
        // 사용자가 검색을 싲가할 때의 초기값으로 사용됩니다.
        searchValue: "korea", // default 검색어

        // API에서 가져온 이미지 데이터를 저장할 배열을 초기화 합니다.
        // 이 배열은 이미지 정보를 담고 있으며, UI에서 이를 렌더링 할 수 있습니다.
        images: [], // 이미지 값
        ...
        perPage: 30, // 무료 버전의 Unsplash API에서는 페이지당 검색되는 이미지 수는 30개가 최대 - 현재('25. 04)
    ...
    actions: {
        ...
        async fetchApi() {
            ...
            // 이 키는 API 호출 시 인증을 위해 필요하며, 보안상 노출되지 않도록 주의해야합니다.
            // .env로 관리하는 연습을 스스로 해보도록 합니다.
            ...
            try {
                ...
                // API에서 받은 총 페이지 수를 totalPages에 저정합니다.
